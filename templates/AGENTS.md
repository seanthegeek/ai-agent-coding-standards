...
- **Name this project's slower or network-dependent suites here, and the code that requires them.** The fresh-context review prompt and the review step in `CLAUDE.md` both read this list to decide what a round has to run. Left empty, a round that should have run the live suite skips it silently and still reports a clean review.
...
Before handing it over, commit every fix: the diff command compares commits, so an uncommitted fix is invisible to the reviewer, and `git status --porcelain` must print nothing. Then bring the base up to date and confirm it moved, with `git fetch origin && git log --oneline origin/main..HEAD`. A fetch never moves the local `main`, which is why the diff command names `origin/main`; a merge base taken from a stale local branch would put already-merged commits into the review. If the fetch fails — offline, a proxy, expired credentials — stop and fix that first, because the review would otherwise run against a base that never moved and nothing in the prompt can detect it.
